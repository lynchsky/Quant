...
def init(context):
    ...
    由于国债期货行情较现券行情密集，可在国债期货价格跌破布林带下轨时买入现券，在国债期货价格突破布林带上轨时卖出现券。
    """
    ...
    context.pre_close = None
    context.pre_upper = None
    context.pre_lower = None
...
def on_bar(context, bar):
    ...
    if bar.cic_code == context.cs2.cic_code:
        ...
        if (len(bar_data) == context.boll_period) and (sum(np.isnan(bar_data)) == 0):
            # 调用talib计算布林带指标
            ...
            # 如果保存的pre_close、pre_upper和pre_lower均不为None值，则进行后续操作
            if (context.pre_close is not None) and (context.pre_upper is not None) and (context.pre_lower is not None):
                # 价格跌破布林下轨时，目标持仓应为多单
                if (context.pre_close > context.pre_lower) and (bar.close < lower[-1]):
                    context.should_have_volume = context.trade_volume

                # 价格突破布林上轨时，目标持仓应为空单
                elif (context.pre_close < context.pre_upper) and (bar.close > upper[-1]):
                    context.should_have_volume = -context.trade_volume

            # 保存pre_close、pre_upper和pre_lower的值，供后续使用
            context.pre_close = bar.close
            context.pre_upper = upper[-1]
            context.pre_lower = lower[-1]
