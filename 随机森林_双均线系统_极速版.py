...
def init(context):
    ...
    为提高胜率，本策略只在机器学习的买卖信号和双均线的买卖信号一致时才进行发单操作。
    ...
    # 接收参数
    context.fast_period = add_parameter(int, 'fast_period', '快速均线周期')
    context.slow_period = add_parameter(int, 'slow_period', '慢速均线周期')
...
def on_bar(context, bar):
    ...
    # 如果机器学习模型已经训练完毕，则根据训练好的预处理模型和机器学习模型进行预测并进行发单操作
        ...
        bar_data = get_bar_n(cic_code=context.cs1.cic_code,
                             interval=context.cs1.interval,
                             count=max(context.fast_period, context.slow_period) + 1,
                             fields=['close'])
        ...
        # 计算快速均线和慢速均线
        fast_ma = talib.SMA(bar_data, timeperiod=context.fast_period)
        slow_ma = talib.SMA(bar_data, timeperiod=context.slow_period)

        # 计算均线金叉死叉信号
        buy_signal = fast_ma[-1] > slow_ma[-1] and fast_ma[-2] < slow_ma[-2]
        sell_signal = fast_ma[-1] < slow_ma[-1] and fast_ma[-2] > slow_ma[-2]
        ...
        # 如果预测下一根K线为阳线且双均线发出买入信号且当前未持有多头仓位，则发出买入信号
        ...
        # 如果预测下一根K线为阴线（阴线包括十字星线）且双均线发出卖出信号且当前未持有空头头寸，则发出卖出信号
