...
def init(context):
    ...
    220205.CFXO      K-Bar      1min    X-BondT+1    是         原始        cs1
    220203.CFXO      K-Bar      1min    X-BondT+1    是         原始        cs2
    220201.CFXO      K-Bar      1min    X-BondT+1    是         原始        cs3
    ...
    计算现券三个不同品种对应曲线过去N个值的价差（短期限 + 长期限 - 2 * 中期限）及价差的75%分位点、50%分位点和25%分位点。
...
def on_bar(context, bar):
    ...
    # # 对获取到的数据进行填充操作
    # short_middle_bar_spread_data = format_data(short_middle_bar_spread_data['close_yield-close_yield'], method='fill')
    ...
    # # 对获取到的数据进行填充操作
    # long_middle_bar_spread_data = format_data(long_middle_bar_spread_data['close_yield-close_yield'], method='fill')
    ...
    if (len(short_middle_bar_spread_data) == len(long_middle_bar_spread_data) == 2) and (
        ...
        # 计算三品种收盘价收益率的价差数据（短期限 + 长期限 - 2 * 中期限）
        ...
        if (len(cs4_curve_data) == len(cs5_curve_data) == len(cs6_curve_data) == context.quantile_period) and (
            ...
            if (context.cs1_close is not None) and (context.cs2_close is not None) and (context.cs3_close is not None):
                ...
                if (not context.flag_75) and (bar_spread_data[-2] < cur_percent_75 < bar_spread_data[-1]):
                    # 卖出价差即买入长期限和短期限品种，卖出中期限品种（由于是用收益率计算价差且是用价格发单，因此发单方向与价差买卖方向相反）
                ...
                if context.flag_75 and (bar_spread_data[-2] > cur_percent_50 > bar_spread_data[-1]):
                    # 买入价差即卖出长期限和短期限品种，买入中期限品种（由于是用收益率计算价差且是用价格发单，因此发单方向与价差买卖方向相反）
