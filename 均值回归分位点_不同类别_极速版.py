...
def init(context):
    ...
    计算不同类别（第一个为现券、第二个为IRS，计算价差时现券使用收益率、IRS使用价格）两个品种过去N根K-BAR的价差及价差的75%分位点、50%分位点和25%分位点。
...
def on_tick(context, tick):
    ...
    # 如果填充后的价差数据的数据量完整且不含NaN值且计算出的50%分位点有值（50%分位点有值，则75%分位点和25%分位点也有值）且保存的现券和IRS TICK的price字段有值，则进行后续操作
