...
def init(context):
    ...
    由于国债期货行情较现券行情密集，可在国债期货对应的双均线金叉时买入现券，在国债期货对应的双均线死叉时卖出现券。
    """
    ...
    # 添加因子
    context.fast_ma = add_factor(float, 'fast_ma', '短期均线')
    context.slow_ma = add_factor(float, 'slow_ma', '长期均线')

    # 添加全局变量
    context.strategy_group = 'A'
    context.should_have_volume = 0
    context.pre_fast_ma = None
    context.pre_slow_ma = None
...
def on_bar(context, bar):
    ...
    if bar.cic_code == context.cs2.cic_code:
        ...
        if (len(bar_data) == context.slow_period) and (sum(np.isnan(bar_data)) == 0):
            # 调用talib计算短期均线和长期均线
            fast_ma = talib.SMA(bar_data, timeperiod=context.fast_period)[-1]
            slow_ma = talib.SMA(bar_data, timeperiod=context.slow_period)[-1]

            # 因子赋值
            context.fast_ma = fast_ma
            context.slow_ma = slow_ma

            # 如果保存的pre_fast_ma和pre_slow_ma均不为None值，则进行后续操作
            if (context.pre_fast_ma is not None) and (context.pre_slow_ma is not None):
                # 双均线金叉时，目标持仓应为多单
                if (context.pre_fast_ma < context.pre_slow_ma) and (fast_ma > slow_ma):
                    context.should_have_volume = context.trade_volume

                # 双均线死叉时，目标持仓应为空单
                elif (context.pre_fast_ma > context.pre_slow_ma) and (fast_ma < slow_ma):
                    context.should_have_volume = -context.trade_volume

            # 保存pre_fast_ma和pre_slow_ma的值，供后续使用
            context.pre_fast_ma = fast_ma
            context.pre_slow_ma = slow_ma
