...
def init(context):
    ...
    该策略训练模型时使用的特征除了K-Bar的原始信息外还包括威廉指标和收盘价的ROC指标。
    ...
    # 接收参数
    context.willr_period = add_parameter(int, 'willr_period', '威廉指标计算周期')
...
def on_bar(context, bar):
    ...
    if context.model is None:
        ...
        # 添加威廉指标特征
        bar_data['WILLR'] = talib.WILLR(bar_data['high'].values,
                                        bar_data['low'].values,
                                        bar_data['close'].values,
                                        timeperiod=context.willr_period)

        # 添加ROC指标特征
        bar_data['ROC'] = talib.ROC(bar_data['close'].values,
                                    timeperiod=context.roc_period)
    ...
    # 如果机器学习模型已经训练完毕，则根据训练好的预处理模型和机器学习模型进行预测并进行发单操作
        ...
        bar_data = get_bar_n(cic_code=context.cs1.cic_code,
                             interval=context.cs1.interval,
                             count=max(context.willr_period, context.roc_period + 1),
        ...
        # 计算威廉指标特征
        willr = talib.WILLR(bar_data['high'],
                            bar_data['low'],
                            bar_data['close'],
                            timeperiod=context.willr_period)

        # 计算ROC指标特征
        roc = talib.ROC(bar_data['close'],
                        timeperiod=context.roc_period)

        # 构造特征
        feature = [[bar.open, bar.high, bar.low, bar.close, bar.open_yield, bar.high_yield, bar.low_yield,
                    bar.close_yield, willr[-1], roc[-1]]]
