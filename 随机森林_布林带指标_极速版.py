...
def init(context):
    ...
    为提高胜率，本策略只在机器学习的买卖信号和布林带的买卖信号一致时才进行发单操作。
...
def on_bar(context, bar):
    ...
    # 如果机器学习模型已经训练完毕，则根据训练好的预处理模型和机器学习模型进行预测并进行发单操作
        ...
        # 计算布林带上下轨
        upper, _, lower = talib.BBANDS(bar_data,
                                       timeperiod=context.boll_period,
                                       nbdevup=context.boll_multiple,
                                       nbdevdn=context.boll_multiple,
                                       matype=0)

        # 计算布林带买卖信号，价格金叉下轨为买入信号，价格死叉上轨为卖出信号
        buy_signal = bar_data[-1] > lower[-1] and bar_data[-2] < lower[-2]
        sell_signal = bar_data[-1] < upper[-1] and bar_data[-2] > upper[-2]
        ...
        # 如果预测下一根K线为阳线且布林带发出买入信号且当前未持有多头仓位，则发出买入信号
        ...
        # 如果预测下一根K线为阴线（阴线包括十字星线）且布林带发出卖出信号且当前未持有空头头寸，则发出卖出信号
